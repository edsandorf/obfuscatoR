...
Once you have created your designs, you might want to save them. We can do this using the `save_design()` function. The function stores the designs in a .csv file or multiple .csv files if you have generated multiple designs. 
...
We can calculate the expected payout to the observer by weighting it with the highest posterior probabilities for each action. We can get the posterior probabilities from our design: 

The expected payout to the decision maker depends on the probability that the observer will try and guess the rule govering their actions. The probability of guessing depends on the expected payout to the observer (calculated above) and the payout to the observer should she refrain from guessing. 
