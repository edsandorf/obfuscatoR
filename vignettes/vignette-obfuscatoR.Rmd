...
Let us assume that the set of rules (motivations above), $R$, and the set of possible actions, $A$, are known to both the decision maker and the observer. Using the notions of information entropy [@Shannon1948] and Bayesian updating, we can formulate the observers best guess as to what motivates the decison maker as the posterior probability in \autoref{eqn-posterior}. This is the probability of a rule conditional on having observed an action. 
